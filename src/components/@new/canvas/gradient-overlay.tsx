  ...
  useEffect(() => {
    if (value && dimensions.width > 0) {
      ...
      if (data) {
        ...
        setPoint1Pos({
          x: clamp(coords.x1, dimensions.width),
          y: clamp(coords.y1, dimensions.height),
        });
        setPoint2Pos({
          x: clamp(coords.x2, dimensions.width),
          y: clamp(coords.y2, dimensions.height),
        });
      }
    }
  }, [value, dimensions]);
  ...
  const generateGradientCSS = () => {
    return type === "linear"
      ? `linear-gradient(${angle + 90}deg, ${stops
          ...
          .join(", ")})`
      : `radial-gradient(circle at ${(center1.x / dimensions.width) * 100}% ${(center1.y / dimensions.height) * 100}%, ${stops
          .map((stop) => {
